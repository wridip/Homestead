...
const EditProperty = () => {
  ...
  const handleRemoveImage = (imageToRemove) => {
    if (imageToRemove.startsWith('blob:')) {
      setSelectedFiles((prev) => prev.filter((file) => URL.createObjectURL(file) !== imageToRemove));
    } else {
      setImagesToDelete((prev) => [...prev, imageToRemove]);
      setFormData((prev) => ({ ...prev, images: prev.images.filter((img) => img !== imageToRemove) }));
    }
    setImagePreviews((prev) => prev.filter((img) => img !== imageToRemove));
  };
  ...
  const validateStep = (step) => {
    ...
    if (step === 1) {
      ...
      else if (isNaN(formData.baseRate) || parseFloat(formData.baseRate) <= 0)
  ...
  if (isLoading) {
    return (
      <div className="container mx-auto max-w-4xl p-8 bg-background rounded-3xl shadow-xl border border-border flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-4 text-muted-foreground">
          <div className="w-10 h-10 rounded-full border-4 border-primary border-t-transparent animate-spin" />
          <p className="text-sm font-semibold uppercase tracking-widest">Loading property…</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto max-w-4xl p-8 bg-background rounded-3xl shadow-xl border border-border">
      ...
      {formErrors.general && (
        <p className="text-red-500 text-sm font-semibold mb-6 px-4 py-3 bg-red-500/10 rounded-2xl border border-red-500/20">
          {formErrors.general}
        </p>
      )}
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  />
                  {formErrors.name && (
                    <p className="text-red-500 text-[10px] font-bold uppercase mt-1 ml-1">{formErrors.name}</p>
                  )}
                </div>
                <div className="space-y-2">
                  ...
                  {formErrors.type && (
                    <p className="text-red-500 text-[10px] font-bold uppercase mt-1 ml-1">{formErrors.type}</p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                />
                {formErrors.address && (
                  <p className="text-red-500 text-[10px] font-bold uppercase mt-1 ml-1">{formErrors.address}</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                {formErrors.description && (
                  <p className="text-red-500 text-[10px] font-bold uppercase mt-1 ml-1">{formErrors.description}</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                />
                {formErrors.contact && (
                  <p className="text-red-500 text-[10px] font-bold uppercase mt-1 ml-1">{formErrors.contact}</p>
                )}
              </div>
            </motion.div>
          )}
                  ...
                  {formData.amenities.filter((a) => !Object.values(amenityCategories).flat().includes(a)).length > 0 && (
                    <div className="mt-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {formData.amenities
                          .filter((a) => !Object.values(amenityCategories).flat().includes(a))
                          .map((amenity) => (
                            <div
                              key={amenity}
                              className="flex items-center gap-2 bg-primary text-primary-foreground px-3 py-1.5 rounded-full text-xs font-semibold shadow-sm"
                            >
                              {getAmenityIcon(amenity)}
              ...
              <section>
                ...
                <div className="space-y-4">
                  {formData.roomTypes.map((room, index) => (
                    <div
                      key={index}
                      className="grid grid-cols-1 sm:grid-cols-5 gap-4 p-5 rounded-2xl bg-card border border-border group relative"
                    >
                      <div className="sm:col-span-2 space-y-1">
                        ...
                        />
                        {formErrors[`roomType-${index}-name`] && (
                          <p className="text-red-500 text-[10px] font-bold">{formErrors[`roomType-${index}-name`]}</p>
                        )}
                      </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          className="w-full bg-transparent border-b border-border focus:border-primary outline-none py-1 text-sm font-semibold"
                        />
                        {formErrors[`roomType-${index}-beds`] && (
                          <p className="text-red-500 text-[10px] font-bold">{formErrors[`roomType-${index}-beds`]}</p>
                        )}
                      </div>
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          className="w-full bg-transparent border-b border-border focus:border-primary outline-none py-1 text-sm font-semibold"
                        />
                        {formErrors[`roomType-${index}-occupancy`] && (
                          <p className="text-red-500 text-[10px] font-bold">{formErrors[`roomType-${index}-occupancy`]}</p>
                        )}
                      </div>
                      <button
                        type="button"
            ...
            >
              <div className="flex flex-col gap-2">
                ...
                <p className="text-muted-foreground text-sm">
                  Help guests find you accurately. Drag the pin or click on the map to update your homestay's coordinates.
                </p>
              </div>
              ...
              {formErrors.location && (
                <p className="text-red-500 text-sm font-bold italic">{formErrors.location}</p>
              )}
            </motion.div>
          )}
            ...
            >
              ...
              <section className="space-y-6">
                ...
                >
                  ...
                  <div className="text-center">
                    <p className="font-bold text-sm">Upload New Photos</p>
                    <p className="text-[10px] text-muted-foreground uppercase tracking-widest mt-1">
                      High quality JPEG/PNG preferred
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {imagePreviews.map((image, index) => (
                    <div key={image} className="aspect-square relative rounded-2xl overflow-hidden group shadow-md">
                      <img
                      ...
                      {index === 0 && (
                        <div className="absolute bottom-0 left-0 right-0 bg-primary/80 text-[8px] font-bold text-center py-1 uppercase text-white tracking-widest">
                          Cover Photo
                        </div>
                      )}
                    </div>
              ...
              <section className="grid grid-cols-1 md:grid-cols-2 gap-8 items-end">
                <div className="space-y-3">
                  ...
                  {formErrors.baseRate && (
                    <p className="text-red-500 text-[10px] font-bold mt-1 ml-1">{formErrors.baseRate}</p>
                  )}
                </div>
