...
const PhotoGallery = ({ images, propertyName }) => {
  ...
  return (
    <div className="relative mb-12 group">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 grid-rows-1 md:grid-rows-2 gap-2 h-[300px] md:h-[500px] overflow-hidden rounded-2xl border border-border shadow-2xl">
        {/* Main large image */}
        ...
        <button 
          ...
          className="absolute bottom-6 right-6 flex items-center gap-2 px-4 py-2 bg-card/90 border border-border hover:bg-muted text-foreground text-sm font-semibold rounded-lg shadow-xl transition-all"
        >
      ...
      {(selectedImage || showAll) && (
        <div
          className="fixed inset-0 bg-background flex flex-col z-[100] animate-in fade-in duration-300"
          onClick={() => { setSelectedImage(null); setShowAll(false); }}
        >
          <div className="flex justify-between items-center p-6 bg-background/80 backdrop-blur sticky top-0 z-10">
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-12 space-y-8" onClick={(e) => e.stopPropagation()}>
            {showAll ? (
              <div className="max-w-4xl mx-auto space-y-8">
                <h2 className="text-3xl font-bold text-foreground mb-12 italic">Photos of {propertyName}</h2>
